...
*Version 54 — Changes from v53: (1) Orphaned_Text_Log.md protocol added: new GitHub file tracking all text cut during editorial passes for potential reuse. Appended incrementally during any A13 or prose pass that produces cuts. Format: chapter/section · hub item ID · date · reason · candidate placement · full original text. Pushed at session end alongside Content_Changes_Log.md. (2) A13 Output Rules updated: when a section cut produces reusable text, Claude appends the cut text and metadata to Orphaned_Text_Log.md before delivering the review document. (3) "Other Project Files" table updated to include Orphaned_Text_Log.md. (4) Affirmative construction rule added to PI v51 session-end TODO — now confirmed in PI (carried from memory). Replaces all prior versions.*
...
**Session close:** Claude delivers updated master, appends Content_Changes_Log.md entry for any chapter where prose changes introduced new citations/endnotes/glossary terms/index candidates, appends Orphaned_Text_Log.md entry for any text cut and preserved for reuse, pushes data.json to GitHub (and index.html if UI changed), outputs push confirmation and continuation prompt. If UI changes shipped this session: update `i_hubui_changelog` in data.json before the final push.
...
**Standing rule — Orphaned_Text_Log (automatic on every cut):** Whenever prose is cut during an A13 or any editorial pass and flagged for potential reuse elsewhere, append an entry to Orphaned_Text_Log.md on GitHub before delivering the review document. Append incrementally; push at session end alongside Content_Changes_Log.md. Format: see Orphaned_Text_Log Protocol below.
...
| "Editorial pass [scope]" | A13 | Unified seven-level editorial read — architecture (Step 0), subheading audit (Step 0b), paragraph logic, transitions, clarity, voice + AI-speak + em-dash, sentence mechanics, and Sterling commercial review — run in a single pass per section. **Delivery rule: 4 or more proposals → .md review document delivered alongside hub fordave items. 3 or fewer proposals → inline chat only.** Scope unit: one chapter per run. Do not combine chapters. Subheading proposals logged to index_audit.md for B7c. Nothing enters master until Dave closes the hub item. Cut text preserved in Orphaned_Text_Log.md before review document is delivered. See A13 Full Protocol below. |
...
- **Cuts with reusable text:** Before delivering the review document, append all cut text to Orphaned_Text_Log.md on GitHub (fetch SHA, append, push).
...
### Orphaned_Text_Log Protocol

**File location:** GitHub repo root — `Orphaned_Text_Log.md`. Fetch via Contents API (SHA first) before appending.

**When to append:** Whenever prose is cut during an A13 or any editorial pass and the cut text is flagged for potential reuse elsewhere in the manuscript. Append before delivering the review document. Push at session end alongside Content_Changes_Log.md.

**Entry format:**

```
## [Section name] — [Hub item ID] — [YYYY-MM-DD]

**Type:** Cut / Preserved for reuse
**Source:** [hub item ID]
**Reason:** [One sentence on why it was cut]
**Candidate placement:** [Where the text might be reused — chapter, section, or "undetermined"]

---

**[Full original text — with heading labels if multiple paragraphs]**

[Text verbatim as it appeared in the manuscript]
```

**B8 consumption:** B8 reads Orphaned_Text_Log.md when processing any chapter flagged for reuse. If a cut passage has been approved for a new location, B8 inserts it there. B8 does not move text autonomously — reuse placements require Dave direction.
...
| Orphaned_Text_Log.md | Preserves text cut during editorial passes for potential reuse. Appended during any A13 or prose pass that produces cuts. Consumed by B8 and by Dave during manual editing. |
...
**Hub UI changelog:** Canonical record at hub item `i_hubui_changelog` (tag: hubui). Update at session end whenever UI changes ship. Enforced via memory.
